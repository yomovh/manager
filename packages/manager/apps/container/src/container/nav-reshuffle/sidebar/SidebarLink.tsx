...
const StaticLink: React.FC<ComponentProps<StaticLinkProps>> = ({
  ...
  return (
    <a
      ...
      id={id}
    >
      {t(short ? node.shortTranslation : node.translation)}
      {node.isExternal && (
        <span
          aria-hidden="true"
      ...
      {!short && <SidebarLinkTag node={node} />}
      {!short && (count as number) > 0 && (
        <span
...
const SidebarLink: React.FC<ComponentProps<SidebarLinkProps>> = ({
  ...
  return !node.children && (node.url || node.routing) ? (
    ...
    <button
      ...
      id={id}
    >
      <span> {t(short ? node.shortTranslation : node.translation)}</span>
      <div>
        {!short && node.children ? (
          <span
